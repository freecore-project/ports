...
The immutable `source/15.0-U1.6` branch contains the complete
FreeCORE release source. It is the directly cloneable build input.
Its history consists only of FreeCORE source snapshot commits, one
per published release; no private development history is reachable.
...
`source/15.0-U1.6` branch in the same repository.
