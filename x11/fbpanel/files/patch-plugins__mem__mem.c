...
+	static int	mib_freecount[4], mib_inactivecount[4],
+			mib_swapinfo[16], init = 0, pagesize;
+	static u_int	mib_free_size, mib_inactive_size,
+			mib_swapinfo_size;
+	struct xswdev	xsw;
+	long		scale;
+	size_t		len;
+	int		freecount, inactivecount, n;
+
+
+	if (init == 0) {
+		//initialize meminfo
...
+	stats.mem.used = (realmem - pagesize * (freecount + inactivecount)) / 1024;
+
+	//get swapinfo
+	scale = pagesize / 1024;
+	for (n = 0;; ++n) {
